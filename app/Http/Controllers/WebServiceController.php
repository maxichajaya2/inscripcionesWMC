...
class WebServiceController extends Controller
{
    ...
    {
        // 1. Función para limpiar caracteres especiales y tildes
        $clean = function ($str) {
            $unwanted = array('Š' => 'S', 'š' => 's', 'Ž' => 'Z', 'ž' => 'z', 'À' => 'A', 'Á' => 'A', 'Â' => 'A', 'Ã' => 'A', 'Ä' => 'A', 'Å' => 'A', 'Æ' => 'A', 'Ç' => 'C', 'È' => 'E', 'É' => 'E', 'Ê' => 'E', 'Ë' => 'E', 'Ì' => 'I', 'Í' => 'I', 'Î' => 'I', 'Ï' => 'I', 'Ñ' => 'N', 'Ò' => 'O', 'Ó' => 'O', 'Ô' => 'O', 'Õ' => 'O', 'Ö' => 'O', 'Ø' => 'O', 'Ù' => 'U', 'Ú' => 'U', 'Û' => 'U', 'Ü' => 'U', 'Ý' => 'Y', 'Þ' => 'B', 'ß' => 'Ss', 'à' => 'a', 'á' => 'a', 'â' => 'a', 'ã' => 'a', 'ä' => 'a', 'å' => 'a', 'æ' => 'a', 'ç' => 'c', 'è' => 'e', 'é' => 'e', 'ê' => 'e', 'ë' => 'e', 'ì' => 'i', 'í' => 'i', 'î' => 'i', 'ï' => 'i', 'ð' => 'o', 'ñ' => 'n', 'ò' => 'o', 'ó' => 'o', 'ô' => 'o', 'õ' => 'o', 'ö' => 'o', 'ø' => 'o', 'ù' => 'u', 'ú' => 'u', 'û' => 'u', 'ü' => 'u', 'ý' => 'y', 'þ' => 'b', 'ÿ' => 'y');
            return strtr($str, $unwanted);
        };
        ...
        $pasesAEnviar = [];
        $nombreCategoria = strtoupper($inscripcion->categoria_inscripcion->nombre_es ?? '');

        // 3. Lógica para determinar si es por días o pase completo
        if ($inscripcion->id_categoria_inscripcion && preg_match('/\bDIA\b/u', $nombreCategoria)) {
            // ES POR DÍAS: Extraemos los días del JSON
            $diasData = is_array($inscripcion->dias) ? $inscripcion->dias : json_decode($inscripcion->dias, true);
            ...
            if ($diasData) {
                foreach ($mapeoDias as $key => $nombreDia) {
                    if (!empty($diasData[$key])) {
                        $pasesAEnviar[] = [
                            'passType'    => "PARTICIPANTE " . $nombreDia,
                            'accessScope' => "single-day"
                        ];
                    }
                }
            }
        ...
        // Si no se detectó la palabra "DIA" o no se marcaron días, se envía como pase completo
        if (empty($pasesAEnviar)) {
            $pasesAEnviar[] = [
                'passType'    => "PARTICIPANTE",
        ...
        // 5. Bucle de envío individual por cada pase generado
        foreach ($pasesAEnviar as $pase) {

            $data_ws = [
                "eventCode"   => 'WMC26',
                "passType"    => $pase['passType'],
                "accessScope" => $pase['accessScope'],
                "holderData"  => [
                    ...
                    "companyName"  => substr(strtoupper($clean($persona->company ?? "NO DEFINIDO")), 0, 50),
                    "position"     => substr(strtoupper($clean($persona->ocupacion->name ?? "NO DEFINIDO")), 0, 50),
                    "country"      => $persona->direccion->pais->codigo_iso ?? "PE"
                ],
            ];

            // dd([
            //     "INFO" => "JSON WMC MULTIEVENTOS A ENVIAR",
            //     "URL_POST" => $this->url_multieventos,
            //     "JSON_ENVIADO" => $data_ws,
            //     "CABECERAS" => $cabecerasProexplo
            // ]);
            ...
            // Almacenamos el resultado de este envío específico
            $resultados[] = [
                'passType' => $pase['passType'],
                'status'   => $statusCode,
                'response' => $response
        ...
        // 6. Manejo de Respuesta Final (Tomando la lógica de errores del primer envío)
        // Evaluamos el status de la última petición para el retorno principal
        $lastStatus = end($resultados)['status'];
        ...
        switch ($lastStatus) {
            ...
            case 201:
                return [
                    'success' => true,
                    'message' => 'Inscripción(es) procesada(s) correctamente.',
                    'data'    => $resultados
                ];
            case 409:
                return [
            ...
            default:
                return [
                    'success' => false,
                    'message' => 'Error en el proceso de Multieventos. Código: ' . $lastStatus,
                    'details' => $resultados
                ];
        }
    }
